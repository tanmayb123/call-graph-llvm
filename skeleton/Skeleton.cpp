...
//    }
      ...
      for (auto &I : B) {
        if (auto *call = dyn_cast<CallInst>(&I)) {
          if (auto *fun = call->getCalledFunction()) {
            ...
            for (set<string>::iterator i = fpointers[defpointer[label]].begin();
                 i != fpointers[defpointer[label]].end(); ++i)
              callgraph[callerName].insert(*i);
          }

        } else if (auto *store = dyn_cast<StoreInst>(&I)) {
          string valueName =
              store->getValueOperand()->stripPointerCasts()->getName();
  ...
  virtual bool doFinalization(Module &M) {
    ...
    for (map<string, set<string>>::iterator i = callgraph.begin();
         i != callgraph.end(); i++) {
      errs() << "[" << i->first << "]: ";
      int j = 0;
      int n = i->second.size();
      for (set<string>::iterator ii = i->second.begin(); ii != i->second.end();
           ++ii) {
        errs() << "[" << *ii << "]";
        if (j != n - 1)
          errs() << ", ";
